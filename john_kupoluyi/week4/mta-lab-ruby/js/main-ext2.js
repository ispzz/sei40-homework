...
const mta = {
  ...
  oneLineTrip: function(line, start, destination){
    ...
    if(startIdx > endIdx){
      return ['north', stations.slice(endIdx, startIdx + 1).reverse()];
    }
    return ['south', stations.slice(startIdx, endIdx + 1)];
  }, //oneLineTrip()
  ...
  printTrip: function(startLine, destLine, map){
    ...
    if(map instanceof Array){
      ...
      //map[0] - direction of travel, map[1] - travel map
      this.generateMap(map[0], map[1], now.getTime());
  ...
  generateMap: function(direction, map, departure){
    let travelTime;

    for(let i = 0; i < map.length; i++){
      let station = map[i];
      let timeToStation = station.timetoStation;

      if(i > 0 && direction === 'north'){
        console.log((map[i-1]));
        timeToStation = map[i - 1].timetoStation;
      }
      travelTime = this.calculateTravelTime(i, timeToStation, departure);
  ...
  calculateTravelTime: function(idx, timeToStation, departure){
    let travelTime;

    if(idx === 0){
      travelTime = this.getTravelTime(departure, ' depart at');
    } else {
      departure += timeToStation * 60000;
