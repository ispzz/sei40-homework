...
s1 = Song.create! title: 'Achy Break Heart'
...
art1.songs << s1
#ActiveRecord is 'repurposing' (overloading) the '<<' operator which Ruby uses as an alias for array.push: here it means "The song s1 has its artist_id set to art1"


# 'Have a Safe Trip, Dear' is by 'June of 44'
art2.songs << s2

#Both 'Burn the Witch'AND 'Indentikit' are by 'Radiohead'
art3.songs << s3 << s4
...
alb3.songs << s3 << s4 # Both 'Burn the Witch' (s3) and 'Indentikit' (s4) are on 'A Moon-Shaped Pool' (alb3)
...
# Because this is many-to-many, you could also start from the genre side:
# g1.songs << s1 << s2 << s3
...
# Add songs to the mixtapes, # IDEA:
